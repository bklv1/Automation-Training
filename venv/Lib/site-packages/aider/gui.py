...
class GUI:
    ...
    def do_sidebar(self):
        with st.sidebar:
            ...
            st.warning(
                "Please provide you test case to generate answer on top of your existing framework."
            )

            st.chat_input("Enter your test cases:")
    ...
    def do_run_java_class(self):
        ...
        if self.button("Run Test Recorder", help=text):
            import subprocess
            result = subprocess.run(['java', '-cp', r'C:\\Users\\tsvetomir.banovski\\Documents\\Learn\\Automation-Training\\src\\main\\java\\', 'TestRecorder'], capture_output=True, text=True)
            self.info(f"Java class output:\n{result.stdout}")
            if result.stderr:
                self.error(f"Java class errors:\n{result.stderr}")
