...
class GUI:
    ...
    def do_run_java_class(self):
        text = "Runs the Java main class"
        if self.button("Run Test Recorder", help=text):
            ...
            classpath = os.path.join(cwd, "src", "main", "java")
            
            # Run the Java command
            result = subprocess.run(['java', '-cp', classpath, 'TestRecorder'], capture_output=True, text=True)
            
            if result.returncode == 0:
                self.info(f"Test Recorder output:\n{result.stdout}")
                self.state.test_recorder_output = result.stdout
            else:
                self.error(f"Test Recorder errors:\n{result.stderr}")
