...
class GUI:
    ...
    def do_sidebar(self):
        with st.sidebar:
            ...
            test_case = st.text_area("Enter your test cases:")
            additional_instructions = st.text_area("Enter additional instructions (Optional)")

            if st.button("Submit"):
                html_page = self.get_test_recorder_output()
                prompt = f"""For this part of html page:\n\n{html_page}\n\nAs a senior automation QA, you have to automate the test scenario using the currently implemented methods and only implement what is missing. Please follow the practices used inside the codebase. Here are the test steps:\n\n{test_case}"""
                ...
                self.prompt = prompt
                self.prompt_as = "user"
                self.process_chat()  # Directly process the chat instead of using st.rerun()
    ...
    def process_chat(self):
        ...
        with self.messages.chat_message("assistant"):
            ...
            if stream is not None:
                ...
                code_snippets = self.extract_code_snippets(res)

                if code_snippets:
                    with self.messages.expander("Suggested Code Snippets"):
                        for i, snippet in enumerate(code_snippets, 1):
                            st.code(snippet, language="python")
                            if st.button(f"Copy Snippet {i}"):
                                st.write("Snippet copied to clipboard!")
                                # You might need to implement a clipboard functionality here
            else:
                st.error("No response received from the AI. Please try again.")
                self.state.messages.append({"role": "assistant", "content": "No response received."})
